...
class AssetGuard extends EventEmitter {
    ...
    static _finalizeForgeAsset(asset, commonPath){
        return new Promise((resolve, reject) => {
            fs.readFile(asset.to, (err, data) => {
                ...
                reject("No se puede finalizar la instalación, version.json no encontrada! Se habra cambiado el formato de 'distribution.json'?")
            })
        })
    }
    ...
    _assetChainIndexData(versionData, force = false){
        ...
        return new Promise((resolve, reject) => {
            ...
            if(!fs.existsSync(assetIndexLoc) || force){
                console.log('Descargando ' + versionData.id + ' asset index.')
    ...
    loadForgeData(server){
        ...
        return new Promise(async (resolve, reject) => {
            ...
            reject('No se encontró el modulo forge!')
        })
    }
            ...
            async.eachLimit(dlQueue, limit, (asset, cb) => {
                ...
                req.on('response', (resp) => {
                    ...
                    if(resp.statusCode === 200){
                        ...
                        req.abort()
                        console.log(`Fallo al descargar ${asset.id}(${typeof asset.from === 'object' ? asset.from.url : asset.from}). codigo ${resp.statusCode}`)
